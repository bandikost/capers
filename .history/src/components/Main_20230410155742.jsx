  ...
  return (
    ...
    <div className="table">
      {sortedBalances.map((entry, index) => (
        ...
        <div className="match" style={{margin: "-30px 0px", fontSize: "15px"}}>последний заряженный матч:
          <p >{team[entry[0]]}</p>
          {editingMatch === entry[0] ? (
            <div>
              <input style={{width: "150px"}}
                type="text"
                value={match[entry[0]]}
                onChange={(e) =>
                  firebase.database().ref(`match/${entry[0]}`).set(e.target.value)
                }
              />
              <button onClick={handleSaveClick}>сохранить</button>
            </div>
